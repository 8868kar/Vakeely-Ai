...
router.post('/search', async (req: Request, res: Response) => {
  try {
    ...
    const results: { act: any; matchingSections: any[] }[] = [];
    acts.forEach((act: any) => {
      const matchingSections = act.sections.filter((section: any) => {
        const sectionText = `${section.title} ${section.description} ${(section.keywords || []).join(' ')}`.toLowerCase();
        return searchTerms.some((term: string) => sectionText.includes(term));
      });
