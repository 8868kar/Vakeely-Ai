...
import { adminAPI, legalAPI } from '../services/api.js';
import { FiUsers, FiShield, FiDatabase, FiActivity, FiCheck, FiX, FiTrash2 } from 'react-icons/fi';
import { ILawyer, IUser, ILegalAct, IAppointment } from '../types/index.js';
...
const AdminPanel: React.FC = () => {
  ...
  const [legalActs, setLegalActs] = useState<ILegalAct[]>([]);
  ...
  const verifyLawyer = async (id: string, status: 'approved' | 'rejected') => {
    try {
  ...
  const deleteLegalAct = async (id: string) => {
                      ...
                      <table className="data-table">
                        ...
                        <tbody>
                          {analytics.recentAppointments.map(apt => (
                            <tr key={apt._id}>
                              <td>{(apt.userId as any)?.name || '-'}</td>
                              <td>{(apt.lawyerId as any)?.name || '-'}</td>
