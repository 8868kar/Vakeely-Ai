...
import { lawyerAPI, appointmentAPI, uploadAPI } from '../services/api.js';
import FileUpload from '../components/FileUpload.js';
import { FiCalendar, FiCheckCircle, FiClock, FiUsers, FiEdit3, FiSave } from 'react-icons/fi';
import { IAppointment, IUser } from '../types/index.js';
...
const LawyerDashboard: React.FC = () => {
  ...
  const [stats, setStats] = useState<DashboardStats>({ total: 0, pending: 0, confirmed: 0, completed: 0 });
  ...
  const fetchDashboardData = async () => {
    try {
      const res = await lawyerAPI.getAppointments(user!._id);
      const apts = res.data;
      setAppointments(apts);
      setStats({
        total: apts.length,
        pending: apts.filter((a: IAppointment) => a.status === 'pending').length,
        confirmed: apts.filter((a: IAppointment) => a.status === 'confirmed').length,
        completed: apts.filter((a: IAppointment) => a.status === 'completed').length,
      });
      setProfileData({
        bio: (user as any).bio || '',
    ...
    <div className="page-content">
      <div className="container">
        <div style={{ marginBottom: 32 }}>
          ...
          {(user as any)?.verified === 'pending' && (
            <div style={{ marginTop: 12, padding: '12px 20px', background: 'rgba(245,158,11,0.1)', border: '1px solid rgba(245,158,11,0.3)', borderRadius: 'var(--radius-sm)', color: 'var(--accent)', fontSize: '0.9rem', display: 'flex', alignItems: 'center', gap: 8 }}>
              <FiClock /> Your profile is pending verification. An admin will review your profile soon.
            </div>
          )}
        </div>
                ...
                {appointments.slice(0, 8).map(apt => (
                  <div key={apt._id} style={{ padding: '14px 16px', background: 'var(--bg-glass)', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border)' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                      <strong style={{ fontSize: '0.9rem' }}>{(apt.userId as any)?.name || 'User'}</strong>
